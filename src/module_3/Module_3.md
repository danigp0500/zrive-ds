...
```python
data_dir = os.path.join(os.getcwd(),'..', 'module_2','data')
...
## Filter data (orders with 5 items or more)


```python
# For each order, we sum all outcomes
...
## Data division
Due to existence of time, we must to do a time-based split
...
## Baseline
We use as baseline the mean popularity of each product


```python
popularity = train.groupby("variant_id")["outcome"].mean()
val = val.copy()
val["global_popularity"] = val["variant_id"].map(popularity).fillna(0)
```
...
```python
X_train = pd.get_dummies(train[feature_cols], columns=["product_type"], drop_first=True)
X_val = pd.get_dummies(val[feature_cols], columns=["product_type"], drop_first=True)
...
```python
val["y_pred_ml"] = model.predict_proba(X_val)[:, 1]
```
...
```python
plot_metrics(val["outcome"], val["global_popularity"], "Baseline: Popularity")
plot_metrics(val["outcome"], val["y_pred_ml"], "Model: Logistic Regression")

```


    
![png](Module_3_files/Module_3_22_0.png)
    



    
![png](Module_3_files/Module_3_22_1.png)
    


In this case, we are justifying using ML since we are overcoming the performance of the popularity.

Now, we are stardardizing because there are variables with huge values `days_since_purchase = 200` and others have small/binary values . In addition, we apply penalitation.

Also, with this preprocessing, we avoid to kill Kernel
...
val["y_pred_lasso"] = model_lasso.predict_proba(X_val_scaled)[:, 1]
```
...
```python
plot_metrics(val["outcome"], val["global_popularity"], "Baseline: Popularity")
plot_metrics(val["outcome"], val["y_pred_lasso"], "Model: Logistic Regression (Lasso)")
```
